...
export class SocketService {
  ...
  getFirstData(){
    ...
    getLogin(path) {
      return this.http.get(BASE_API + path, this.jwt()).map((response: Response) => response.json());
    }

    postLogin(path, formData) {
    return this.http.post(BASE_API + path, formData).map((response: Response) => response.json());
    }
